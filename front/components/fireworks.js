      ...
      setEmojiList(
        (prev) =>
          prev
            ...
            .filter(
              (emoji) =>
                emoji.y < window.innerHeight * 2 &&
                emoji.t < 50 &&
                emoji.x > -200 &&
                emoji.x < window.innerWidth + 200,
            ), // Limites de y
      );
    };
    ...
    const interval = setInterval(() => {
      ...
      function get_gonk() {
        ...
        const initialy = Math.floor(
          Math.random() * (screen.height - screen.height / 2) +
            screen.height / 2,
        );
        const newEmoji = {
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden">
      ...
      {showCodyHelmet ? (
        ...
        >
          <img
            src={"/icon/codyChallenge/CodyButton.png"}
            onDragStart={(e) => {
              handleClick(e);
            }}
            onDrop={(e) => {
              ...
              <div>
                ...
                <div className="flex items-center justify-center sm:flex sm:items-start pb-3">
                  <div className={`text-lg leading-6 font-medium`}>
                    <p
                      className={`text-justify ${darkMode ? "text-gray-200" : "text-gray-900"}`}
                    >
                      Salutations, individu qui fait des tickets ou qui a vu sur
                      discord qu'il y a un mini-jeu aujourd'hui sur MyFab.
                    </p>
                    <p
