...
CREATE TABLE IF NOT EXISTS `users` ( `i_id` INT NOT NULL AUTO_INCREMENT , `v_firstName` VARCHAR(75) NOT NULL , `v_lastName` VARCHAR(75) NOT NULL , `v_email` VARCHAR(75) NOT NULL , `v_password` VARCHAR(64) NOT NULL , `dt_creationdate` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP , `dt_lastUsed` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP , `i_idschool` INT DEFAULT NULL , `i_schoolyear` INT(1) DEFAULT NULL , `i_idschoolprevious` INT DEFAULT NULL , `i_schoolyearprevious` INT(1) DEFAULT NULL , `v_discordid` VARCHAR(18) NULL DEFAULT NULL , `v_language` VARCHAR(2) NOT NULL DEFAULT 'fr' , `dt_ruleSignature` DATETIME NULL DEFAULT NULL , `b_deleted` BOOLEAN NOT NULL DEFAULT FALSE , `b_visible` BOOLEAN NOT NULL DEFAULT TRUE , `b_mailValidated` BOOLEAN NOT NULL DEFAULT FALSE , `b_isMicrosoft` BOOLEAN NOT NULL DEFAULT FALSE , `b_darkMode` BOOLEAN NOT NULL DEFAULT FALSE , `v_title` VARCHAR(254) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NULL DEFAULT NULL , `dt_rickrolled` DATETIME DEFAULT NULL , `v_internalNote` TEXT DEFAULT NULL , PRIMARY KEY (`i_id`), CONSTRAINT uk_v_email Unique (`v_email`)) ENGINE = InnoDB;

CREATE TABLE IF NOT EXISTS `gd_school` ( `i_id` INT NOT NULL AUTO_INCREMENT , `v_name` VARCHAR(25) NOT NULL , PRIMARY KEY (`i_id`), UNIQUE KEY `uk_v_name` (`v_name`)) ENGINE = InnoDB;
ALTER TABLE `users` ADD CONSTRAINT `fk_u_idSchool` FOREIGN KEY IF NOT EXISTS (`i_idSchool`) REFERENCES `gd_school`(`i_id`) ON DELETE RESTRICT ON UPDATE RESTRICT;
ALTER TABLE `users` ADD CONSTRAINT `fk_u_idSchoolprevious` FOREIGN KEY IF NOT EXISTS (`i_idschoolprevious`) REFERENCES `gd_school`(`i_id`) ON DELETE RESTRICT ON UPDATE RESTRICT;
...
CREATE TABLE IF NOT EXISTS `gd_printmaterial` ( `i_id` INT NOT NULL , `v_name` VARCHAR(30) NOT NULL , `i_idRoleNotify` INT(3) NOT NULL , PRIMARY KEY (`i_id`), CONSTRAINT uk_v_name Unique (`v_name`)) ENGINE = InnoDB;
ALTER TABLE `gd_printmaterial` ADD CONSTRAINT `fk_gdr_idRole` FOREIGN KEY IF NOT EXISTS (`i_idRoleNotify`) REFERENCES `gd_roles`(`i_id`) ON DELETE RESTRICT ON UPDATE RESTRICT;
...
CREATE TABLE IF NOT EXISTS `printstickets` ( `i_id` INT NOT NULL AUTO_INCREMENT , `i_idUser` INT NOT NULL , `i_useridschool` INT(11) NULL DEFAULT NULL , `i_userschoolyear` INT(1) NULL DEFAULT NULL , `i_projecttype` INT(3) NOT NULL , `i_groupNumber` VARCHAR(10) NULL DEFAULT NULL , `v_idChannel` VARCHAR(18) NULL , `dt_creationdate` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP , `dt_modificationdate` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP , `i_status` INT(4) NULL DEFAULT NULL , `i_priority` INT(4) NOT NULL , `i_material` INT(4) NOT NULL , `v_agentNote` TEXT DEFAULT NULL , `b_isDeleted` BOOLEAN NOT NULL DEFAULT FALSE , PRIMARY KEY (`i_id`)) ENGINE = InnoDB;
...
ALTER TABLE `printstickets` ADD CONSTRAINT `fk_pt_idSchool` FOREIGN KEY IF NOT EXISTS (`i_useridschool`) REFERENCES `gd_school`(`i_id`) ON DELETE RESTRICT ON UPDATE RESTRICT;
...
ALTER TABLE `printstickets` ADD CONSTRAINT `fk_pm_idMaterial` FOREIGN KEY IF NOT EXISTS (`i_material`) REFERENCES `gd_printmaterial`(`i_id`) ON DELETE RESTRICT ON UPDATE RESTRICT;
ALTER TABLE `printstickets` ADD COLUMN IF NOT EXISTS `b_pinned` BOOLEAN NOT NULL DEFAULT FALSE;
ALTER TABLE `printstickets` ADD COLUMN IF NOT EXISTS `v_agentNote` TEXT DEFAULT NULL;
ALTER TABLE `printstickets` MODIFY `i_groupNumber` VARCHAR(10) NULL DEFAULT NULL;
ALTER TABLE `users` ADD COLUMN IF NOT EXISTS `v_internalNote` TEXT DEFAULT NULL;
CREATE TABLE IF NOT EXISTS `log_userschange` ( `i_id` INT NOT NULL AUTO_INCREMENT , `i_idUserAdmin` INT NOT NULL , `i_idUserTarget` INT NOT NULL , `v_action` VARCHAR(25) NOT NULL , `v_newValue` VARCHAR(255) , `dt_timeStamp` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP , PRIMARY KEY (`i_id`)) ENGINE = InnoDB;
CREATE TABLE IF NOT EXISTS `notifications` ( `i_id` INT NOT NULL AUTO_INCREMENT , `i_idUser` INT NOT NULL , `v_message` VARCHAR(255) NOT NULL , `v_link` VARCHAR(100) NULL , `b_read` BOOLEAN NOT NULL DEFAULT FALSE , `dt_creationDate` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP , PRIMARY KEY (`i_id`)) ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `ticketmessages` ( `i_id` INT NOT NULL AUTO_INCREMENT , `i_idUser` INT NOT NULL , `i_idTicket` INT NOT NULL , `v_content` VARCHAR(1100) NOT NULL , `dt_creationDate` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP , PRIMARY KEY (`i_id`)) ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `ticketfiles` ( `i_id` INT NOT NULL AUTO_INCREMENT , `i_idUser` INT NOT NULL , `i_idTicket` INT NOT NULL , `v_fileName` VARCHAR(200) NOT NULL , `v_fileServerName` VARCHAR(256) NOT NULL , `v_comment` VARCHAR(256) NOT NULL DEFAULT '' , `b_valid` BOOLEAN , `i_idprinter` INT DEFAULT NULL, `dt_creationDate` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP , `dt_modificationDate` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP , `b_deleted` BOOLEAN NOT NULL DEFAULT FALSE , PRIMARY KEY (`i_id`)) ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `usercookies` ( `i_id` INT NOT NULL AUTO_INCREMENT , `i_idUser` INT NOT NULL , `v_value` VARCHAR(64) NOT NULL , `dt_creationDate` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP , `dt_lastUsed` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP , `dt_expireDate` DATETIME , PRIMARY KEY (`i_id`)) ENGINE = InnoDB;
ALTER TABLE `usercookies` ADD CONSTRAINT `fk_uc_idUser` FOREIGN KEY IF NOT EXISTS (`i_idUser`) REFERENCES `users`(`i_id`) ON DELETE RESTRICT ON UPDATE RESTRICT;

CREATE TABLE IF NOT EXISTS `log_connection` ( `i_id` INT NOT NULL AUTO_INCREMENT , `i_idUser` INT(11) NOT NULL , `v_userAgent` VARCHAR(256) NOT NULL , `v_browser` VARCHAR(128) NOT NULL , `v_ip` VARCHAR(16) NOT NULL , `dt_creationDate` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP , PRIMARY KEY (`i_id`)) ENGINE = InnoDB;
ALTER TABLE `log_connection` ADD CONSTRAINT `fk_lc_idUser` FOREIGN KEY IF NOT EXISTS (`i_idUser`) REFERENCES `users`(`i_id`) ON DELETE RESTRICT ON UPDATE RESTRICT;
...
CREATE TABLE IF NOT EXISTS `stats` ( `i_id` INT NOT NULL AUTO_INCREMENT , `v_name` VARCHAR(32) NOT NULL , `i_value` INT(10) NOT NULL DEFAULT '1' , `v_year` VARCHAR(4) NOT NULL , PRIMARY KEY (`i_id`)) ENGINE = InnoDB;
